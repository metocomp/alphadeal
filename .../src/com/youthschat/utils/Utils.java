...
public final class Utils {
	 ...
	 * Formats USA phone.
	 */
	public static String formatUsaPhone(String phone) {
		if (phone == null) {
			return "";
		}
		StringBuilder formated = new StringBuilder();
		if (!phone.startsWith("1") && !phone.startsWith("+1")) {
			formated.append("1").append(phone);
		} else {
			formated.append(phone);
		}
		return formated.toString().replace(" ","").replace("+", "").replace("-", "");
	}
	...
	public final static CharSequence formatTime(Context context, String time) {
		return DateUtils.getRelativeDateTimeString(context, Long.parseLong(time) * 1000, DateUtils.MINUTE_IN_MILLIS, DateUtils.WEEK_IN_MILLIS,0);
	}
	
	/**
	 * Returns the map of country name to country dial number i.e. +86
	 */
	public final static Map<String, String> populateCountries(Context context, Spinner spinner) {
		...
		for( Locale loc : locale ){
		    ...
		    if( country.length() > 0 && !countries.contains(country) ){
		        ...
		        countryCodeMap.put(country, country2phone.get(loc.getCountry()));
		    }
		}
