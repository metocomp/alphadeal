...
//    				EntityManagerHelper.log("finding WcUser instance with property: " + propertyName + ", value: " + value, Level.INFO, null);
	...
	public List<WcUser> findAll(
		WcUser me, final int...rowStartIdxAndCount
		) {
					EntityManagerHelper.log("finding all WcUser instances that are not my friend yet", Level.INFO, null);
			try {
			final String queryString = "select model from WcUser model where model.userId != ?1 and model.userId != 10000 order by model.userId desc";
								Query query = getEntityManager().createQuery(queryString);
								query.setParameter(1, me.getUserId());
					if (rowStartIdxAndCount != null && rowStartIdxAndCount.length > 0) {	
						int rowStartIdx = Math.max(0,rowStartIdxAndCount[0]);
						if (rowStartIdx > 0) {
							query.setFirstResult(rowStartIdx);
	...
	public List<WcUser> findByFriend(WcUser me, final int...rowStartIdxAndCount) {
		// TODO Auto-generated method stub
		EntityManagerHelper.log("getting friends", Level.INFO, null);
		try {
			final String queryString = "select model from WcUser model where model.userId != 10000 and model IN ?1 order by model.userId desc";
							Query query = getEntityManager().createQuery(queryString);
